...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      const allInvoices = await db
        .select({
          ...
          paymentProof: invoices.paymentProof,
  ...
  app.get("/api/client/invoices/:id/download", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const doc = new PDFDocument();
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename=Factura_${invoice.invoiceNumber || invoice.id}.pdf`);
      doc.pipe(res);

      doc.fontSize(20).text('FACTURA', { align: 'center' });
      doc.moveDown();
      doc.fontSize(12).text(`Número: ${invoice.invoiceNumber || 'S/N'}`);
      doc.text(`Estado: ${invoice.status}`);
      doc.text(`Fecha de Vencimiento: ${new Date(invoice.dueDate).toLocaleDateString()}`);
      doc.moveDown();
      doc.text(`Cliente: ${invoice.clientLegalName || 'N/A'}`);
      doc.text(`RUC/CI: ${invoice.clientRuc || 'N/A'}`);
      doc.moveDown();
      doc.text(`Proyecto: ${invoice.projectName || 'N/A'}`);
      doc.text(`Descripción: ${invoice.description || 'N/A'}`);
      doc.moveDown();
      doc.fontSize(14).text(`Monto Total: USD ${parseFloat(invoice.amount).toLocaleString()}`, { align: 'right' });
